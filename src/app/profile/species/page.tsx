        ...
        {submissions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-yellow-50">
              <Sprout className="h-8 w-8 text-yellow-500" />
            </div>
            <p className="font-medium text-gray-600">Você ainda não sugeriu nenhuma erva</p>
            ...
            {submissions.map((sp) => {
              ...
              return (
                <div key={sp.id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-1 text-xs text-gray-400">Origem: {ORIGIN_LABEL[sp.origin]}</p>
